...
class CustomSearchDelegate extends SearchDelegate<String> {
  ...
  Future<void> fetchData() async {
    try {
      ...
      final res2 = await Trip().getTrips();
      searchList = res['data']
          .map<String>((e) => e['placeName'].toString().toLowerCase())
          .toList();
      searchList.addAll(res2['data'].map<String>((e) => e['tripName'].toString().toLowerCase()).toList());
      idList = res['data'].map<String>((e) => e['_id'].toString()).toList();
      idList.addAll(res2['data'].map<String>((e) => e['_id'].toString()).toList());
    } catch (e) {
      ...
      itemBuilder: (context, index) {
        return ListTile(
          ...
          onTap: () {
            ...
            Navigator.pushNamed(
              context, RoutesName.placeDataScreen, arguments: idList[index],
            );
      ...
      itemBuilder: (context, index) {
        return ListTile(
          ...
          onTap: () {
            ...
            Navigator.pushNamed(
              context, RoutesName.placeDataScreen, arguments: idList[index1],
            );
          },
        );
      },
